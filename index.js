const mypokemon = document.getElementById('listPokemon')
...
.then(pokemon => {
    const getData = document.importNode(template.content, true);
        ...
        getData.querySelector('p').textContent = pokemon.name;
        getData.querySelector('#ability').textContent = pokemon.abilities.map(ability => ability.ability.name);
